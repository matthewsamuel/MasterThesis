...
class KrigingDataCombination(vbdeDataCombination):
    def __init__(self, input_path, missing_geohashes, sel_freq):
        ...
        # Raw events DataFrame for event-level Shapley
        ...
        raw["event_id"] = raw.index
        self.events_df = raw[["event_id", "user_id", "geohash", "longitude", "latitude", "value", "date"]]
        ...
        # Build correlation matrix in constructor
        self.build_correlation_matrix()

    def add_distance_rank(self, df):
        """Add subindex per geohash_x sorted by distance and geohash_y"""
        # First sort by geohash_x, then by distance, then by geohash_y
        df_sorted = df.sort_values(['geohash_x', 'distance', 'geohash_y'])
        
        # Create subindex incrementing per geohash_x group
        df_sorted['subindex'] = df_sorted.groupby('geohash_x').cumcount() + 1
        
        return df_sorted

    def build_correlation_matrix(self):
        """Build the correlation matrix between all neighbor geohashes."""
    ...
    def get_neighbors(self, target_geohash):
        """Return neighbors for a target, sorted by distance + subindex."""
        ...
        sorted_df["subindex"] = range(1, len(sorted_df) + 1)  # Assign subindex
        return sorted_df  # DataFrame with columns [geohash_y, distance, subindex, ...]
    ...
    def getCombinedInput(self, target_geohash, K_event_ids):
        # Get target coordinates
        target_row = self.df1[self.df1["geohash"] == target_geohash].iloc[0]
        target_coords = (target_row["longitude"], target_row["latitude"])
        # Build neighbor info per geohash based on events in coalition
        neighbor_info = []
        indices = []
        # Get unique neighbor geohashes and distances
        neighbor_df = self.combined[
            self.combined["geohash_x"] == target_geohash
        ][["geohash_y", "distance"]].drop_duplicates()
        for _, row in neighbor_df.iterrows():
            gh = row["geohash_y"]
            dist = row["distance"]
            events_in_gh = self.events_df[
                (self.events_df["geohash"] == gh) &
                (self.events_df["event_id"].isin(K_event_ids))
            ]
            if events_in_gh.empty:
                continue
            mean_val = events_in_gh["value"].mean()
            count = len(events_in_gh)
            ...
            indices.append(self.geohash_to_index[gh])
        covariance_subset = self.correlation_matrix[np.ix_(indices, indices)]
        return {
            "target": target_coords,
            "neighbors": neighbor_info,
            "covariance_subset": covariance_subset
        }
    ...
    def getGlobalCovariance(self):
        """Returns the full covariance matrix for all known geohashes."""
        return self.correlation_matrix

    def getSources(self, target_geohash):
        # Return event IDs for all events in the neighbor geohashes of the target
        neighbor_geohashes = self.combined[
            self.combined["geohash_x"] == target_geohash
        ]["geohash_y"].unique().tolist()
        neighbor_events = self.events_df[
            self.events_df["geohash"].isin(neighbor_geohashes)
        ]
        return neighbor_events["event_id"].tolist()
...
class KrigingModel(vbdeModel):
    def __init__(self, combiner, target_geohash, use_shapley_mode=False):
        ...
        self.selected_subindices = None  # Only relevant for Shapley
        ...
        if not self.use_shapley_mode:
            ...
            # Shapley mode: Delayed initialization
            # ---------------------------------------
            self.neighbors_df = None  # Will be set during getModelOutput()
            self.C = None
            self.D = None
...
class vbdeFramework:
    ...
    """
        ...
        Only clear model outputs; preserve Shapley buffer vS
    """
    ...
    def evaluateTuple(self, K):
        ...
        if len(K) == 0:
            if self.isVSbuffered:
                self.vS.at[str(K)] = self.ValueFunction.defaultValue
            return self.ValueFunction.defaultValue
        if self.isCalculatedYet(K):
            return self.getBufferValue(K)
        else:
            try:
                # Get Shapley subset (dictionary with neighbors, covariances, ...)
                yInput = self.Combiner.getCombinedInput(self.Model.target_gh, K)
    ...
    def executeWorks(self, daskClient, iWorkerProcess_init, nWorkers):
        ...
        i = 0           # Which is the work sequence number for the workers
        jWorker = 0     # Which is the worker to which we will schedule a work for
        # It removes duplicates if there are any
        existingWorks = []
        for work in self.queueS:
            if not work in existingWorks: # This line avoids including the tuple
                ...
                if (jWorker < nWorkers -1):
                    ...
                    jWorker = 0
                    i += 1
        # Stores the series in the corresponding worker schedules
        for worker in schedule.columns:
            schedule[worker].to_csv(f'worker/worker_{worker}_schedule.csv', index=False)

        futures = []
        for worker in schedule.columns:
            futures.append(daskClient.submit(iWorkerProcess_init,
                                            worker,
                                            self.Combiner,
                                            self.Model,
                                            self.ValueFunction,
    ...
    def WorkerProcess_init(self, i, combiner, model, value_fn):
        # combiner, model, value_fn are passed directly as arguments
        # Create a fresh framework instance for this worker
        ...
        # Evaluate each task, but don't abort on failure
        for s in tasks:
            if isinstance(s, str) and s.startswith('('):
                try:
                    ...
                    continue
        # Export partial results, ignoring export errors
        try:
    ...
    def consolidateWorkerFiles(self, nWorkers):
        for i in list(range(nWorkers)):
            self.importVSFile(os.path.join('worker', f'worker_{i}_output_vS.csv'))
            #print(vS)
            #self.importYFile('worker '+str(i)+' output y.csv')
            #print(y)
    ...
    def getSV_TSS(self, N, r, TruncationValue,debug=False):
        ...
        total_perms = len(dfPermutations)
        print(f"[DEBUG] getSV_TSS: starting {total_perms} permutations for N={len(N)} players, r={r}")
        
        for i, P in dfPermutations.items():
            if i % max(1, total_perms // 10) == 0:
                print(f"[DEBUG] getSV_TSS progress: permutation {i+1}/{total_perms}")
            ...
            for j, player in enumerate(P):
                ...
                vSeval = vSui
        end_time = time.time()
        print(f"[DEBUG] getSV_TSS finished in {end_time - start_time:.2f}s, nExecs={nExecs}")
    ...
    def processPermutations_parallel(self, daskClient, iWorkerProcess_init, N,\
        ...
        while pos < len(N):
                ...
                for i in range(r * len(N)):
                    ...
                    if vSeval[i] <= TruncationValue:
                        if self.isCalculatedYet(Sui):
                            ...
                            existingWorks = True
            # Early exit if no work scheduled in this chunk
            if not existingWorks:
                break
            ...
            # Clean up buffers to release memory before next chunk
            self.clearBuffer()
            import gc
            gc.collect()
    ...
    # original code without batches
    # def processPermutations_parallel(self, daskClient, iWorkerProcess_init, N,\
    #                                 r, TruncationValue, dfPermutations):
    #     # Counter of the number of executions done
    #     nExecs = 0
        
    #     # Structured sampling loop - evaluating SV for each player in each 
    #     #permutation. The improvement allows to save in v(S) calculations, 
    #     #since we evaluate the uniform shuffles we created before.
    #     t = 0
    #     j = 0
    ...
    #     #  We will evaluate the rx|N| permutations in parallel and launch a set 
    #     # of valuation works for each position j. It ensures that all the
    #     # the required values are calculated.
    #     #  The condition of exit from the loop are two:
    #     #   - Either we reach the end of the permutations
    #     #   - Or there is not pending work, meaning that all permutations reached
    #     #   the truncation value
    #     while j < len(N) and existingWorks:
    #         existingWorks = False
            
    #         i = 0
            
    #         # processes permutation i
    #         # print("Processing position "+str(j))
            
    #         # 1st loop: Prepare required works
    #         while i<r*len(N):
    #             Sui = tuple(sorted((dfPermutations.at[i])[0:j+1]))
    ...
    #         # Executes the works if needed and updates vSEval[i] for each
    #         #permutation.
    #         if existingWorks:
    #             # Dynamically determine number of workers from the Dask client
    #             nWorkers = len(config.daskClient.cluster.workers)
    #             # Removes duplicates from queueS before executing          
    #             self.executeWorks(daskClient, iWorkerProcess_init, nWorkers)
    #             # 2nd loop: update vSEval[i]
    #             i = 0
    #             while i<r*len(N):
    #                 Sui = tuple(sorted((dfPermutations.at[i])[0:j+1]))
    ...
    #             # Check if vS has reached the truncation value
    #             # Omly process v(Sui) if v(S) is velow the truncation value
    #             if vSeval <= TruncationValue:
    #                 # This is just a search in the v(S) database now
    #                 vSui = self.getBufferValue(Sui)
    #             else:
